...
def parse_arguments():
    ...
    parser.add_argument(
        '-dd', '--detailed-differences',
        type=str,
        default='detailed_differences.csv',
        help='Output CSV file for detailed differences specifying which columns differ. Default: detailed_differences.csv'
    )
...
def initialize_dask_client(scheduler, n_workers, memory_limit):
    """
    ...
    try:
        if scheduler == 'threads':
            cluster = LocalCluster(
                ...
                memory_limit=memory_limit,  # Optional: Specify memory limit
...
def compare_large_csv(args):
    ...
    client = initialize_dask_client(args.scheduler, args.n_workers, args.memory_limit)
    ...
    try:
        ...
        with ProgressBar() if not args.no_progress else nullcontext():
            merged = df1.merge(
                ...
                left_index=True,    # Specify that merge is on index
                right_index=True
            )

        # Log the columns of the merged DataFrame for debugging
        ...
        if common.shape[0].compute() == 0:
            ...
            # Generate Detailed Differences Report
            logging.info("Generating detailed differences report...")
            with ProgressBar() if not args.no_progress else nullcontext():
                # For common rows with differences
                common_diff = merged[merged['_merge'] == 'both']
                if common_diff.shape[0].compute() > 0:
                    # Identify differing columns
                    differing_rows_dd = common_diff[diff_mask]

                    # Select columns with suffixes
                    file1_cols = [col for col in differing_rows_dd.columns if col.endswith('_file1')]
                    file2_cols = [col for col in differing_rows_dd.columns if col.endswith('_file2')]

                    # Prepare DataFrame for detailed differences
                    # Reset index to have place_id as a column
                    differing_rows_dd = differing_rows_dd.reset_index()

                    # Convert to Pandas DataFrame for row-based operations
                    differing_rows_dd_pd = differing_rows_dd.compute()

                    # Define function to find differences in each row
                    def find_differences(row, compare_columns):
                        diffs = []
                        for col in compare_columns:
                    ...
                    # Apply the function to each row and collect differences
                    # Note: This can be memory-intensive for very large datasets
                    # Therefore, consider using compute on a subset if needed

                    # Convert to Pandas for easier manipulation (assuming differing_rows_dd is manageable)
                    try:
                        detailed_diff_list = differing_rows_dd_pd.apply(lambda row: find_differences(row, compare_columns), axis=1).tolist()

                        # Flatten the list with place_id and differences
                        detailed_diffs = []
                        for idx, diffs in enumerate(detailed_diff_list):
                            place_id = differing_rows_dd_pd.iloc[idx][args.key_columns[0]]
                            for diff in diffs:
                        ...
                        # Create DataFrame from the detailed differences
                        detailed_diffs_df = pd.DataFrame(detailed_diffs)

                        if not detailed_diffs_df.empty:
                            logging.info(f"Exporting detailed differences to {args.detailed_differences}")
                            detailed_diffs_df.to_csv(args.detailed_differences, index=False)
                        else:
                            logging.info("No detailed differences found to export.")
                    except Exception as e:
                        logging.error(f"An unexpected error occurred during detailed differences generation: {e}")
                else:
                    logging.info("No common rows to compare for detailed differences.")
